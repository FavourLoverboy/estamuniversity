    ...
    <?php 
        ...
        if($_POST){
            ...
            if($_SESSION['validate_email']){
                ...
                if($emailCheck){
                    $tblquery = "SELECT * FROM staff WHERE email = :email && password = :password";
                    $tblvalue = array(
                        ':email' => htmlspecialchars($email),
                    ...
                    if($login){
                        $tblquery = "SELECT * FROM staff WHERE email = :email AND status = :status";
                        $tblvalue = array(
                            ':email' => htmlspecialchars($email),
                            ':status' => '1'
                        );
                        ...
                        if($statusCheck){
                            foreach($statusCheck as $data){
                                ...
                                if($_SESSION['level'] == 'SUA'){
                                    echo "<script>  window.location='superadmin/dashboard' </script>";
                                }elseif($_SESSION['level'] == 'SA'){
                                    echo "<script>  window.location='studentadmin/dashboard' </script>";
                                }elseif($_SESSION['level'] == 'PA'){
                                    echo "<script>  window.location='paymentadmin/dashboard' </script>";
                                }elseif($_SESSION['level'] == 'RA'){
                                    echo "<script>  window.location='resultadmin/dashboard' </script>";
                                }
                            }
                        }else{
